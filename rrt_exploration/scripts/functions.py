...
class robot:
    ...
    def __init__(self, name):
        ...
        self.plan_service = rospy.get_param(
            '~plan_service', '/move_base/NavfnROS/plan') #/move_base_node/NavfnROS/make_plan
        ...
        self.client = actionlib.SimpleActionClient(
            self.name+'/move_base', MoveBaseAction) #/move_base
...
def index_of_point(mapData, Xp):
    resolution = mapData.info.resolution + 0.01
...
def informationGain(mapData, point, r):
    ...
    r_region = int(r/(mapData.info.resolution+0.01))
    init_index = index-r_region*(mapData.info.width+1)
    for n in range(0, 2*r_region+1):
        start = n*mapData.info.width+init_index
        end = start+2*r_region
        limit = ((start/(mapData.info.width+0.01))+2)*mapData.info.width
